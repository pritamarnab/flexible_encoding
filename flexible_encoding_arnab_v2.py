...
# hyperparameters
HIGH_Value=1e5   # the value to pad with, see function get_elec_data for details 
...
def get_elec_data(subject, df, ecogs, conv_name, all_onsets, offset, lags, elec_id, lag_number,taking_words=True):
    ...
    len_to_pad_high_value= np.shape(np.where(index_onsets < all_onsets[0]))[1]
...
elec_data=custom_Zscore(elec_data)
